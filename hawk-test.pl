...
my $version = '0.57';		# version string
...
# these should be after the select command or they will NOT work
...
our $log_me2 = $conn->prepare('INSERT INTO failed_log ( ip, "user", service, broot ) VALUES ( ?, ?, ?, ? ) ') 
	or die "DIE: Unable to prepare log query: $!\n";
...
# start the real work
# everything bellow this line is executed for every line that enters any of the logs
# keep this as simple as possible in order to stay light
while (<LOGS>) {
	my $brute = 'false';	# define that this line is not brute force 
 	if ( $_ =~ /ssh/ && $_ =~ /Failed/ ) {
	## check ssh
	# match theese lines
	...
	#May 16 03:27:24 serv01 sshd[25536]: Failed password for invalid user suport from ::ffff:85.14.6.2 port 52807 ssh2
		...
		$ip =~ s/::ffff://;
		# do not log the failures from server's local IP address
		next if ( $ip =~ /$myip/ );
		...
		$log_me2->execute($ip, $user, 'ssh', $brute)
			or logger("Failed to insert: ssh $ip $user ($DBI::errstr)");

        } elsif ( $_ =~ /cpanelpop/ ) {
	#May 16 02:37:31 serv01 cpanelpop[29746]: Session Closed host=67.15.172.8 ip=67.15.172.8 user=root realuser= totalxfer=47
                my @pop3 = split /\s+/, $_;
			...
			logger("IP $pop3[7] failed to identify to cppop") if ($debug);
			# check for brureforce
			while ( my ($k,$v) = each (%pop3_faults) ) {
				if ( $v > $max_attempts ) {
					log_broot('pop3', $k, $v); 
			...
			$log_me2->execute($pop3[7], '', 'pop3', $brute) 
				or logger("Failed to insert: cppop $pop3[7] ($DBI::errstr)");
                }
	...
	#May 11 03:58:40 serv01 pop3d: LOGIN FAILED, user=kate, ip=[::ffff:72.43.28.210]
		...
		logger("IP $pop3[8]($pop3[7]) failed to identify to courier-pop3") if ($debug);
		# check for brureforce
		while ( my ($k,$v) = each (%pop3_faults) ) {
			if ( $v > $max_attempts ) {
				log_broot('pop3', $k, $v);
		...
		$log_me2->execute($pop3[8], $pop3[7], 'pop3', $brute)
				or logger("Failed to insert: courier-pop $pop3[8] $pop3[7] ($DBI::errstr)");
	...
	#May 17 17:06:44 serv01 imapd[32199]: Login failed user=dsada domain=(null) auth=dsada host=[85.14.6.2]
		...
		$log_me2->execute($imap[10], $imap[7], 'imap', $brute)
				or logger("Failed to insert: cPanel imap $imap[10] $imap[7] ($DBI::errstr)");
	} elsif ( $_ =~ /imapd/ && $_ =~ /FAILED/ ) {
	# Courier IMAP
	#May 15 05:26:16 serv01 imapd: LOGIN FAILED, user=admin, ip=[::ffff:67.15.243.20]
		...
		$log_me2->execute($imap[8], $imap[7], 'imap', $brute)
				or logger("Failed to insert: courier-imap $imap[8] $imap[7] ($DBI::errstr)");
	...
	#May 16 03:06:43 serv01 pure-ftpd: (?@85.14.6.2) [WARNING] Authentication failed for user [mamam]
		...
		$log_me2->execute($ftp[5], $ftp[11], 'ftp', $brute) 
			or logger("Failed to insert: ftp $ftp[5] $ftp[11] ($DBI::errstr)");
	} elsif ( $_ =~ / 401 / && $_ =~ /GET / ) {
	#85.14.6.2 - root [05/16/2007:07:58:28 -0000] "GET / HTTP/1." 401 0 "" "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.8.1.1) Gecko/20061208 Firefox/2.0.0.1"
		...
		$log_me2->execute($cpanel[0], $cpanel[2], 'cpanel', $brute)
				or logger("Failed to insert: cPanel $cpanel[0] $cpanel[2] ($DBI::errstr)");
   	} else {
		next;
	}
#	check_broot();
